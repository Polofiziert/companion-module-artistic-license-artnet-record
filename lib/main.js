...
const { ArtnetRecorder } = require('./recorder')
...
class ArtnetInstance extends InstanceBase {
	...
	async init(config) {
		...
		this.log('debug', 'main/ArtnetInst/init: ProcessId: ' + process.pid);
		this.log('debug', 'main/ArtnetInst/init: ProcessId: ' + process.platform);
	...
	async configUpdated(config) {
		...
		this.config = config
		this.log('debug', 'main/ArtnetInst/confUpdated: config: ' + JSON.stringify(config))


		if (this.transitions) this.transitions.stopAll()
		this.transitions = new Transitions(this.data, this.config.timer_fast || TIMER_FAST_DEFAULT, this.do_send.bind(this), this)
	...
	async destroy() {
		...
		this.RecorderInstance.closeSockets()
	...
	do_send() {
		...
		if (this.client) {
			this.client.send(this.data, function(self, err, res){
				//this.log('error', 'main/ArtnetInst/do_send: artNet Client thrown error when sending: ' + err + '; On Result: ' + res + '; On Data: ' + self)
			}.bind(this))
		}
	}
