...
class ArtnetRecorder {
    constructor(self){
        this.self = self
        this.dmxData = []
        this.Client = self.client; //IP Port Universe
        this.port = 6454
    ...
    init(){
        ...
        fs.readdir(this.recordDir, function (err, files) {
            if(!err){
        		...
        		this.self.log('debug', 'main/recorder/init: fs readDir error: ' + err)
        		this.self.log('debug', 'main/recorder/init: fs readDir error: try to create dir')

                fs.mkdir(this.recordDir,function(err){
                        if (!err) {
                    		this.self.log('debug', 'main/recorder/init: fs makeDir: Directory Created')
                        }else{
        		            this.self.log('error', 'main/recorder/init: fs makeDir error: ' + err)
                        }
                    }.bind(this)); 
            }
        }.bind(this))
