class Transitions {
	...
	constructor(data, tickMs, sendFcn, InstanceBase) {
		this.data = data
		this.tickMs = tickMs
	...
	stopAll() {
		this.InstanceBase.log('debug', 'main/transitions/stopAll: stopAll called')
	...
	run(channel, to, duration) {
		this.InstanceBase.log('debug', 'main/transitions/run: run called')
		const from = this.data[channel]
		//console.debug(`Run fade on "${channel}" from "${from}" to "${to}" over "${duration}"ms`)
		this.InstanceBase.log('debug', `main/transitions/run: Run fade on "${channel}" from "${from}" to "${to}" over "${duration}"ms`)
